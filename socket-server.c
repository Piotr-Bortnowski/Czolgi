...
Wall map_walls[NUM_WALLS] = {
    {200, 150, 50, 300},  // Ściana pionowa
    {400, 100, 200, 50},  // Ściana pozioma górna
    {500, 400, 200, 50}   // Ściana pozioma dolna
};

// Definicja punktów startowych dla max 4 graczy w rogach mapy (X, Y)
// Uwzględnia margines od krawędzi ekranu oraz rozmiar czołgu (40x40)
float spawn_points[MAX_PLAYERS][2] = {
    { 40.0f,  40.0f},   // Gracz 0: Lewy górny róg
    {720.0f,  40.0f},   // Gracz 1: Prawny górny róg
    { 40.0f, 520.0f},   // Gracz 2: Lewy dolny róg
    {720.0f, 520.0f}    // Gracz 3: Prawy dolny róg
};

// Dodatkowe stany serwera (nie przesyłane do klienta)
...
// Funkcja sprawdzająca czy dwa prostokąty się nakładają (AABB)
...
void *connection_handler(void *arg) {
    ...
    while (recv(sock, &input, sizeof(ClientInput), 0) > 0) {
        ...
        client_inputs[id] = input; // Aktualizacja wciśniętych klawiszy gracza
    ...
    while(1) {
        ...
        for(int i = 0; i < MAX_PLAYERS; i++) {
            if(game_state.tanks[i].active) {
                ...
                // Aktualizujemy wektor kierunku tylko, jeśli gracz wciska jakikolwiek klawisz ruchu
                if (dx != 0.0f || dy != 0.0f) {
                    // Normalizacja wektora, aby po przekątnej nie strzelać szybciej
                ...
                if(client_inputs[i].shoot && shoot_cooldown[i] == 0) {
                    for(int b = 0; b < MAX_BULLETS; b++) {
                        if(!game_state.bullets[b].active) {
                            ...
                            // Wyśrodkowanie pocisku względem czołgu
        ...
        for(int b = 0; b < MAX_BULLETS; b++) {
            if(game_state.bullets[b].active) {
                ...
                // 1. Sprawdzanie wylotu poza ekran
                ...
                // 2. Kolizje pocisków ze ścianami
                int hit_wall = 0;
                for(int w = 0; w < NUM_WALLS; w++) {
                ...
                // 3. Kolizje pocisków z graczami
                for(int p = 0; p < MAX_PLAYERS; p++) {
                    // Sprawdzamy czy trafiliśmy AKTYWNEGO gracza, który NIE JEST właścicielem pocisku
                    if(game_state.tanks[p].active && game_state.bullets[b].owner_id != p) {
                        if(check_collision(game_state.bullets[b].x, game_state.bullets[b].y, bullet_size, bullet_size,
                            ...
                            game_state.bullets[b].active = 0; // Pocisk ulega zniszczeniu
                            game_state.tanks[p].hp -= 20;     // Zabieramy 20 HP
                            
                            if(game_state.tanks[p].hp <= 0) {
                                game_state.tanks[p].active = 0; // Śmierć gracza
                            }
                            break;
                        }
                    }
